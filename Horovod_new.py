...
from keras.layers import Activation, Dense, Dropout, LSTM, LayerNormalization, Bidirectional
...
# Read all the data
total_data = pd.read_csv('VeChain.csv')
...
max_time = 32   # The unfolded time slices of the LSTM Model
...
model = tf.keras.models.Sequential([
    ...
    Bidirectional(LSTM(lstm_neurons, return_sequences=True), input_shape=(max_time, n_input)),
    Dropout(dropout),
    LayerNormalization(epsilon=1e-6),
    
    Bidirectional(LSTM(lstm_neurons)),
    Dropout(dropout),
    LayerNormalization(epsilon=1e-6),
...
# Train the model. callbacks=callbacks
# Horovod: adjust number of steps based on number of GPUs.
model.fit(train_data, train_label, steps_per_epoch=500 // hvd.size(), epochs=epochs, verbose=verbose)
